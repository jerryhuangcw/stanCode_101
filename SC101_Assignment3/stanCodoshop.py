...
def get_average(pixels):
    ...
    for i in range(len(pixels)):
        rgb[0] += pixels[i].red
        rgb[1] += pixels[i].green
        rgb[2] += pixels[i].blue
        count += 1
    rgb[0] //= count  # red avg
    rgb[1] //= count  # green avg
    rgb[2] //= count  # blue avg
    return rgb
...
def get_best_pixel(pixels):
    ...
    best_dist = float('inf')  # shortest distance
    best_pixel = 0
    for i in range(len(pixels)):
        dist = get_pixel_dist(pixels[i], rgb[0], rgb[1], rgb[2])
        if dist < best_dist:
            best_pixel = pixels[i]
            best_dist = dist  # update
    return best_pixel
...
def solve(images):
    ...
    for x in range(result.width):
        for y in range(result.height):
            img_pixel = []
            for i in images:
                pixel = i.get_pixel(x, y)  # same pixels in different images
